...
func follow(host: Node2D, _delta: float) -> void:
...
func leave_lie() -> Node2D:
	...
	decoy = Node2D.new()
	decoy.name = "Lie"
	var _ghost := Figure2D.attach(decoy, "kitsune", Color(0.91, 0.48, 0.22, 0.45))
...
func sidestep(away_from: Vector2) -> void:
	var dir := (global_position - away_from).normalized()
	if dir.length() < 0.1:
		dir = Vector2.RIGHT.rotated(randf() * TAU)
	global_position += dir * 64.0
	if figure:
		figure.set_facing(dir)
